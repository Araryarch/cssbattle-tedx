...
export default function SocialsPage() {
  ...
  const clanChat = useClanChat(
    activeServerId !== "home" ? activeServerId : null,
    user,
  );
  const dmChat = useChat(
    activeServerId === "home" &&
      !["friends", "pending", "add", "global"].includes(activeChannelId)
      ? activeChannelId
      : null,
    user,
  );
  ...
  const joinVoice = async (channelId: string) => {
    try {
      ...
      await api.post("/clans/voice/join", {
  ...
  return (
    <div className="flex h-[calc(100vh-64px)] overflow-hidden bg-black font-sans text-zinc-100">
      ...
      <main
        className={`flex-1 flex flex-col min-w-0 bg-black transition-opacity duration-200 ${mobileSidebarOpen ? "opacity-20 pointer-events-none" : "opacity-100"}`}
      >
        {/* Mobile Menu Trigger */}
        <button
        ...
        {activeServerId === "home" &&
        ["friends", "pending", "add"].includes(activeChannelId) ? (
          /* FRIENDS DASHBOARD */
          <FriendsDashboard
            activeChannelId={activeChannelId}
          ...
          /* CHAT INTERFACE */
          <ChatInterface currentChat={currentChat} user={user} />
        ) : (
