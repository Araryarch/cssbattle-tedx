...
import { getContestSolutionsAction } from "@/lib/submission-actions";
import { getChallengeAction } from "@/lib/challenge-actions";
import { Loader2, ArrowLeft, Code, Hash, Clock, Target, Lock } from "lucide-react";
...
export default function ContestSolutionsPage() {
    ...
    async function loadData() {
        ...
        try {
            const [challengesResult, solutionsResult] = await Promise.all([
                ...
                getContestSolutionsAction(contestId, challengeId)
            ]);
            
            setChallenge(challengesResult);
            if (solutionsResult.authorized) {
                setSolutions(solutionsResult.solutions.map((s: any) => ({ ...s, createdAt: new Date(s.createdAt) })));
            } else {
                setReason(solutionsResult.reason || "Unable to load solutions");
            }
        } catch (e) {
  ...
  if (reason) {
       return (
        <div className="min-h-screen bg-[#050505] p-8 flex flex-col items-center justify-center text-center space-y-6">
            ...
            <p className="text-zinc-500 max-w-md">
                {reason === "Contest in progress" 
                    ? "Solutions are hidden while the contest is active. Please wait until the contest ends."
                    : reason}
            </p>
            <Link 
                href={`/contest/${contestId}/battle/${challengeId}`}
      ...
      <div className="max-w-6xl mx-auto space-y-8 animate-in fade-in slide-in-from-bottom-4 duration-500">
        ...
        <div className="flex items-center justify-between">
            <div className="flex items-center gap-4">
                <Link href={`/contest/${contestId}/battle/${challengeId}`} className="p-2 -ml-2 hover:bg-white/5 rounded-full transition-colors text-zinc-400 hover:text-white group" title="Back to Battle">
                    <ArrowLeft className="w-5 h-5 group-hover:-translate-x-1 transition-transform" />
                </Link>
                <div>
                   <h1 className="text-3xl font-black tracking-tighter text-white flex items-center gap-3">
                     Contest Solutions <span className="text-zinc-500 font-normal text-xl">for {challenge?.title || "Challenge"}</span>
                   </h1>
                   <p className="text-zinc-500 text-sm mt-1">
                       Submission history for this contest. Only high-accuracy solutions ({">="} 75%) are shown.
                   </p>
                </div>
            </div>
            <div className="text-xs font-mono text-zinc-500 border border-white/5 px-3 py-1.5 rounded-lg bg-white/5">
                {solutions.length} qualifying solutions
            </div>
        </div>
        ...
        <div className="flex flex-col gap-4 max-w-5xl mx-auto">
            {solutions.length === 0 ? (
                <div className="py-20 text-center text-zinc-500 italic bg-white/5 rounded-2xl border border-white/5">
                    No solutions met the criteria ({">="} 75% accuracy) during the contest.
                </div>
            ) : (
                solutions.map((sol) => (
                    <div 
